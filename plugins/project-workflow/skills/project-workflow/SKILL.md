...
- Superpowers plugin: `brainstorming`, `writing-plans`는 setup의 질문/계획 보강에만 조건부로 쓰고, `tdd`, `subagent-driven-development` 같은 구현 primitive는 `feature-workflow`로 넘긴다.
...
- raw idea나 새 프로젝트에서는 `grill-me`/`grill-with-docs`와 `office-hours` 질문이 끝나기 전 PRD, issue backlog, architecture handoff를 진행하지 않는다. 사용자가 “추정해서 진행”이라고 명시한 경우에만 fallback으로 진행한다.
...
원본 `grill-me`/`office-hours`를 호출했거나 fallback 질문을 끝내기 전에는 setup 산출물을 확정하지 않는다. 최소 질문은 아래와 같다.
