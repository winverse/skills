...
| Markdown 문서를 4컷/6컷 comic 이미지로 바꾼다 | `markdown-to-comic` | `$imagegen`으로 `comic/comic.png`를 먼저 만들고, `comic/comic.html` wrapper로 이미지, transcript, source anchor, alt text, 검증 기록을 묶는다. | `browser-qa`, `design-review` |
...
| `markdown-to-comic` | Markdown 문서의 핵심 개념, 판단 흐름, 절차를 `ComicBrief`로 줄인 뒤 `$imagegen`으로 4컷 또는 6컷 raster comic을 만든다. 대상 폴더의 `comic/comic.png`를 먼저 만들고, 같은 폴더의 `comic/comic.html` wrapper에 이미지, transcript, source anchor, alt text, prompt summary, 검증 기록을 함께 둔다. 공유 스킬 대상이면 `skills/<skill-name>/comic/` 아래에 산출한다. 명령어, 보안 규칙, 정확한 정책 문구는 이미지 안에만 넣지 않는다. | [SKILL.md](../skills/markdown-to-comic/SKILL.md) · [skill.html](../skills/markdown-to-comic/skill.html) |
