...
| 새 스킬의 사람용 HTML guide를 만든다 | `skill-to-html` | `SKILL.md`를 기준 원문으로 보고 사용 조건, 핵심 계약, 금지 경계, 관련 파일, 검증 명령만 한국어 우선 정적 요약으로 압축한다. PC desktop 기준만 보며 버튼, script, 애니메이션, 외부 의존을 넣지 않는다. `browser-qa`, `design-review` |
...
| Skill | 설명 | 자세히 |
| --- | --- | --- |
| `skill-to-html` | `SKILL.md` 옆에 사람이 빠르게 이해할 수 있는 PC desktop 기준, 한국어 우선 정적 요약 `skill.html`을 만든다. Markdown 기준 원문에서 heading/list/table/code/link 의미를 읽고 사용 조건, 핵심 계약, 금지 경계, 파일, 검증 명령만 짧게 압축한다. mobile/tablet layout과 responsive breakpoint는 비대상이다. 버튼, inline JavaScript, animation, 외부 CDN/script/image를 쓰지 않고, Markdown `raw HTML`/event handler/`javascript:` link/외부 script를 그대로 통과시키지 않는다. | [SKILL.md](../skills/skill-to-html/SKILL.md) · [skill.html](../skills/skill-to-html/skill.html) |
