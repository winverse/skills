...
- Use $skill-to-html at `skills/skill-to-html/SKILL.md` whenever a skill is created, installed, forked, or updated, so the skill folder gets a 도표 중심 `skill.html` beside `SKILL.md`; write visible explanations as Korean-first sentences, keep English only for exact identifiers or common acronyms, optimize only for PC desktop, ignore mobile/tablet layout, and check SVG arrow endpoints, wide table placement, overflow, and text overlap when the visual structure changes.
...
- When using `skill-update`, read `docs/update-source-registry.md` first, treat `.gitmodules` as the canonical source for vendored plugin/submodule update candidates, verify `.gitmodules`, `docs/plugin-catalog.md`, and `Plugin update list` drift with `scripts/validate-plugins.ts`, keep `SKILL.md`, references, validators, `agents/openai.yaml`, `skill.html`, snippets, docs, and history aligned when behavior changes, verify found or suspected sources with `web-research`, and do not update plugin packages, MCP config, or submodules incidentally through `skill-update`; when the user explicitly includes plugin updates, use the plugin update lane and keep `.gitmodules`, plugin catalog, registry list, validators, and history aligned; regenerate the `show-skills` HTML catalog after skill add/remove/rename/archive/restore operations.
...
- When using `project-workflow`, treat it as a source-labeled setup skill: choose only needed primitives from Matt Pocock skills, GStack plugin, Superpowers plugin, design-direction, and repo-local custom helpers; mark each as `selected`, `invoked`, `skipped`, or `fallback`. If the current runtime exposes the original skill/plugin/command, invoke selected primitives instead of merely imitating them. Label borrowed names with their source package, for example Matt Pocock skills `grill-me`, GStack plugin `office-hours`, Superpowers plugin `brainstorming`, repo-local custom `project-structure`, and user custom `design.md`.
...
- When `project-workflow` handles a raw idea, new project, or major initiative, complete the Matt Pocock `grill-me`/`grill-with-docs` domain interview and GStack `office-hours` product challenge before PRD, issue backlog, or architecture handoff. If those originals are not callable in the current runtime, mark `fallback`, ask the local interview questions, and record unanswered items in `workflow-state.md`.
...
- `project-workflow` is a repo-owned plugin at `plugins/project-workflow/`. It owns the canonical bundled initial setup skill `project-workflow`; top-level `skills/project-workflow` remains a compatibility entry for existing project snippets. `feature-workflow` stays a separate repeated implementation skill.
...
- Use `skills/skill-to-html/SKILL.md` when creating or revising a skill's `skill.html`; keep visible explanations as Korean-first sentences, optimize only for PC desktop, ignore mobile/tablet layout, keep wide scope tables out of narrow 2-column layouts, and make SVG arrows terminate at visible nodes.
- Update that skill's `skill.html` when adding, removing, renaming, or materially changing a skill.
- Make `skill.html` 도표 중심: include 판단 매트릭스, 흐름도, 차트, 파일 관계도, or 입출력 구조 instead of only splitting text into panels.
