...
- Use $markdown-to-html at <skills-root>/skills/markdown-to-html/SKILL.md whenever a Markdown skill document should become a short, safe, Korean-first `skill.html` beside `SKILL.md`.

## Project-Specific Overrides

- Run this immediately after `skill-creator` initializes or updates a skill.
- Treat `SKILL.md` as the source document and `skill.html` as a human reading view.
- Use skill HTML mode by default: update only the adjacent `skill.html` for the target skill folder. Use general Markdown document mode only when the user explicitly gives a Markdown input and output location.
- Parse Markdown semantics first: headings, lists, tables, code fences, blockquotes, and links.
- Convert Markdown through a small `MarkdownHtmlModel` before writing HTML; do not copy source paragraphs or raw Markdown line breaks directly into layout.
...
- Keep the first screen focused on document name, one-line purpose, input, output, and 3-5 important criteria.
- 일반 설명어는 한국어로 쓴다.
...
- Visible sentences should contain only meaning, criteria, boundaries, and commands extracted from the source document.
