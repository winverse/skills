...
이 스킬은 기존 스킬을 바꿀 때 `docs/update-source-registry.md`를 단일 source registry로 읽고, `SKILL.md`, references, validators, `agents/openai.yaml`, `skill.html`, snippets, docs, history를 하나의 package로 맞춘다. 원본 skill이나 upstream primitive에서 파생된 스킬은 원본을 먼저 확인하고, 원본 delta를 이 repo에 적용한다.
...
- 사용자가 원본 조사를 따로 말하지 않아도 먼저 `docs/update-source-registry.md`를 읽고 source id와 source family를 고른다.
...
- dependency update sweep은 `.gitmodules`의 vendored plugin 전부, repo-owned plugin 목록, `docs/update-source-registry.md`의 workflow primitive lane, 그리고 `project-workflow`와 `feature-workflow`의 usage map을 자동 점검 범위에 넣는다. 이전 호환 표현으로는 `project-workflow`와 `feature-workflow`의 `upstream-dependency-map.md`를 점검 범위에 넣는다는 뜻이다.
...
- `scripts/validate-source-registry.ts`는 workflow usage map의 source id가 `docs/update-source-registry.md`에 존재하는지, usage map에 source URL이나 checked date가 복사되지 않았는지 검사한다.
- workflow provenance-only primitive의 source URL, checked date, upstream version/commit, local 판단은 `docs/update-source-registry.md`를 canonical source로 읽는다. 각 workflow의 `references/upstream-dependency-map.md`는 source id의 usage map으로만 읽고, `.gitmodules` 항목처럼 취급하지 않는다.
...
- dependency update sweep 요청은 plugin update를 명시한 것으로 간주하되, 각 submodule bump 전에는 upstream release/changelog와 local manifest drift를 확인하고 `docs/update-source-registry.md`에 source ledger를 남긴다.
...
- 원본 조사 결과는 source URL, checked date, upstream version/commit, local 채택 판단을 포함해 `docs/update-source-registry.md`의 source ledger로 남긴다.
...
- 원본을 직접 수정할 수 없으면 `docs/update-source-registry.md`에 source ledger를 남기고, references나 history에는 source id와 local delta 이유만 남긴다.
...
| upstream/original delta | provenance preflight, registry source ledger, release/changelog 비교, 원본 변경, local package, history/eval |
| dependency update sweep | `.gitmodules` plugin 전체, repo-owned plugin, `docs/update-source-registry.md` source id 전체, workflow usage map, registry source ledger, plugin catalog/history/validator |

## workflow

1. 대상 skill과 결과를 확인한다.
2. 현재 skill folder, snippets, README, AGENTS, history를 본다.
3. `docs/update-source-registry.md`를 읽고 source id, `.gitmodules`, workflow primitive lane, repo-owned skill inventory 중 어느 source family가 relevant한지 고른다.
...
8. `docs/update-source-registry.md`의 source ledger에 source URL, checked date, upstream version/commit, 변경 후보, local 적용 판단을 남긴다.
...
11. dependency update sweep이면 workflow primitive lane도 함께 실행해 `docs/update-source-registry.md`의 upstream source와 exact primitive name을 먼저 확인하고, `project-workflow`와 `feature-workflow`의 adopted role, handoff boundary, `upstream-dependency-map.md`, snippets, eval, history를 갱신한다.
